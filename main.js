...
// ---- GAME STATE ----
const state = {
  energy:         80,      // 0-100, depletes over time
  maxEnergy:      100,
  population:     1,
  photosynthRate: 0,
  nutrientRate:   0,
...
// =====================================================
// BACKGROUND PLANKTON PARTICLE FIELD (Week 1 feel)
// 15,000 drifting green particles filling the water column
// =====================================================
const PLANKTON_COUNT = 15000;
const planktonPositions = new Float32Array(PLANKTON_COUNT * 3);
const planktonVelocities = new Float32Array(PLANKTON_COUNT * 3);
const planktonPhases = new Float32Array(PLANKTON_COUNT);

for (let i = 0; i < PLANKTON_COUNT; i++) {
  // Spread across entire world volume
  const r = Math.random() * WORLD_RADIUS;
  const theta = Math.random() * Math.PI * 2;
  planktonPositions[i*3]   = Math.cos(theta) * r;
  planktonPositions[i*3+1] = (Math.random() - 0.5) * WORLD_HEIGHT;
  planktonPositions[i*3+2] = Math.sin(theta) * r;

  // Very slow random drift
  planktonVelocities[i*3]   = (Math.random() - 0.5) * 0.018;
  planktonVelocities[i*3+1] = (Math.random() - 0.5) * 0.008;
  planktonVelocities[i*3+2] = (Math.random() - 0.5) * 0.018;

  planktonPhases[i] = Math.random() * Math.PI * 2;
}

const planktonGeo = new THREE.BufferGeometry();
planktonGeo.setAttribute(
  'position',
  new THREE.BufferAttribute(planktonPositions, 3).setUsage(THREE.DynamicDrawUsage)
);
...
const planktonParticles = new THREE.Points(planktonGeo, planktonMat);
planktonParticles.frustumCulled = false;
scene.add(planktonParticles);

function updatePlanktonParticles(time) {
  const pos = planktonPositions;
  const vel = planktonVelocities;
  for (let i = 0; i < PLANKTON_COUNT; i++) {
    pos[i*3]   += vel[i*3];
    pos[i*3+1] += vel[i*3+1] + Math.sin(time * 0.25 + planktonPhases[i]) * 0.003;
    pos[i*3+2] += vel[i*3+2];

    // Wrap vertically
    if (pos[i*3+1] > SURFACE_Y)  pos[i*3+1] = FLOOR_Y;
  ...
  // Color shifts with depth — greener near surface, blue-green at depth
  if (cellMesh) {
    const depthFrac = THREE.MathUtils.clamp(
      (SURFACE_Y - cellMesh.position.y) / WORLD_HEIGHT, 0, 1
    );
    planktonMat.color.setHSL(
      THREE.MathUtils.lerp(0.28, 0.48, depthFrac),
      0.85,
      THREE.MathUtils.lerp(0.60, 0.30, depthFrac)
    );
    planktonMat.opacity = THREE.MathUtils.lerp(0.72, 0.35, depthFrac);
  }
}
...
// Start near surface so photosynthesis works immediately
...
function createNutrientPatch() {
  const geo = new THREE.SphereGeometry(Math.random()*1+0.8, 8, 8);
...
function createDeadZone(x, y, z, radius) {
  ...
  mesh.userData.damage = 10; // energy/sec lost inside
  scene.add(mesh);
  deadZones.push(mesh);
}

createDeadZone(60, -40, 40, 10);
createDeadZone(-55, -30, -50, 8);
...
// PLAYER MOVEMENT
// =====================================================
// Start right at the surface where light is maximum
cellMesh.position.set(0, SURFACE_Y - 2, 0);
...
function updateBiology(delta) {
  ...
  // depthFrac 0 = surface, 1 = floor. Light zone is top 30% of column.
  const lightAvailable = Math.max(0, 1 - depthFrac * 3.3);
  state.photosynthRate = lightAvailable * 6; // up to 6 energy/sec at surface
  ...
  // 0.8/sec means ~125 seconds to drain from full with NO light or nutrients
  state.energy -= 0.8 * delta;
