...
function updatePlanktonParticles(time) {
  ...
  // Color shifts subtly: greener near surface (photosynthesizing), 
  // dimmer and more blue-green at depth
  const depthFrac = THREE.MathUtils.clamp(
    (SURFACE_Y - cellMesh.position.y) / WORLD_HEIGHT, 0, 1
  );
  planktonMat.color.setHSL(
    THREE.MathUtils.lerp(0.28, 0.48, depthFrac), // green → teal
    0.85,
    THREE.MathUtils.lerp(0.60, 0.30, depthFrac)  // bright → dim
  );
...
createDeadZone(30, -20, 15, 10);
createDeadZone(-40, 10, -30, 8);
...
// PLAYER MOVEMENT
// =====================================================
// Start near the surface where light is available
cellMesh.position.set(0, SURFACE_Y - 8, 0);
